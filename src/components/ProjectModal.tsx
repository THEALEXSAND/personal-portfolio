					...
					>
						<article>
							<header className="flex justify-between items-start gap-x-4">
								<div>
									<h3 className="mb-4 text-2xl font-semibold">{selectedProject.name}</h3>
									<p className="max-w-4/5 mb-4 opacity-80 text-sm">{selectedProject.description}</p>
								</div>
							...
							<main className="grid grid-cols-2 gap-8 py-10 border-y border-slate-700">
								...
								<div className="flex flex-col gap-y-3">
									...
									<section>
										...
										<ul className="flex flex-wrap gap-x-2 items-center [&>li:not(*:hover)]:[background-color:_transparent_!important]">
...
interface ImagesSectionProps extends Pick<Project, 'images'> { }
	...
	return (
		<section>
			<picture className="aspect-video relative">
				<img className="rounded-md" src={currentImageUrl} alt="Imagen Principal del Proyecto" />
				...
				<section className="absolute inset-0 flex p-2 justify-between items-center gap-x-4 [&:hover>*]:opacity-100">
					...
					>
						<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="M18.464 2.114a1 1 0 0 0-1.033.063l-13 9a1.003 1.003 0 0 0 0 1.645l13 9A1 1 0 0 0 19 21V3a1 1 0 0 0-.536-.886M17 19.091L6.757 12L17 4.909z" /></svg>
					</button>
					...
					>
						<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="M5.536 21.886a1 1 0 0 0 1.033-.064l13-9a1 1 0 0 0 0-1.644l-13-9A.998.998 0 0 0 5 3v18a1 1 0 0 0 .536.886M7 4.909L17.243 12L7 19.091z" /></svg>
					</button>
				</section>
			</picture>
				...
				{images.map((url, index) => (
					<li className="max-w-25 cursor-pointer" key={url}>
						<picture onClick={() => setCurrentImageUrl(url)}>
							<img
								className={`rounded-md ${currentImageUrl === url ? "border-2 border-primary" : ""
									}`}
								src={url}
